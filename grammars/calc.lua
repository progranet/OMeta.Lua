...
local EvalOTCalc = OMeta.Grammar({_grammarName = 'EvalOTCalc', eval = OMeta.Rule({behavior = function (input)
return input:applyWithArgs(input.grammar.choice, function (input)
local opr, __pass__
...
EvalOTCalc:merge(MixedOTCalc)
EvalOTCalc.primexp = OMeta.Rule({behavior = function (input)
return input:applyWithArgs(input.grammar.choice, function (input)
...
print(ast)
print(EvalOTCalc.eval:matchMixed(ast))
...
local EvalAstCalc = OMeta.Grammar({_grammarName = 'EvalAstCalc', eval = OMeta.Rule({behavior = function (input)
return input:applyWithArgs(input.grammar.choice, function (input)
local opr, __pass__
__pass__, opr = input:applyWithArgs(input.grammar.andPredicate, BinOp)
if not (__pass__) then
...
EvalAstCalc:merge(MixedAstCalc)
local calc = function (...)
return MixedAstCalc.exp:matchMixed(...)
end
local eval = function (...)
return EvalAstCalc.eval:matchMixed(...)
end
...
return {Calc = Calc, EvalCalc = EvalCalc, TableTreeCalc = TableTreeCalc, BinOp = BinOp, OpTreeCalc = OpTreeCalc, MixedOTCalc = MixedOTCalc, EvalOTCalc = EvalOTCalc, AddOp = AddOp, SubOp = SubOp, MulOp = MulOp, DivOp = DivOp, AstCalc = AstCalc, MixedAstCalc = MixedAstCalc, EvalAstCalc = EvalAstCalc}
