...
local MixedOTCalc = OMeta.Grammar({_grammarName = 'MixedOTCalc', primexp = OMeta.Rule({behavior = function (input)
          ...
          return input:applyWithArgs(input.grammar.choice, function (input)
              if not (input:applyWithArgs(input.grammar.object, nil, function (input)
                    return input:applyWithArgs(input.grammar.choice, function (input)
                        _pass, left = input:applyWithArgs(input.grammar.property, 'left', input.grammar.eval)
                        if not (_pass) then
                          return false
                        end
                        _pass, right = input:applyWithArgs(input.grammar.property, 'right', input.grammar.eval)
                        return _pass, right
                      end)
                  end)) then
                return false
          ...
          return input:applyWithArgs(input.grammar.choice, function (input)
              if not (input:applyWithArgs(input.grammar.object, nil, function (input)
                    _pass, operator = input:applyWithArgs(input.grammar.property, 'operator', input.grammar.anything)
