...
|`$result` / `$^`|The result of the current Sequence|Available only if there is [result binding](#result-binding), e.g.:<br>`^:. [? type($^) == 'string' and #($^) == 1]`|
|`$index`|The position (index) of the input stream||
