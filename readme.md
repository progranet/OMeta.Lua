...
|<p align="left">**[Binding](#binding)**:</p>`variable:exp`<br>`num:[10]`<br>`{; prop:=string }`<br>`$var:exp`<br>`$^:exp` / `$result:exp`|<p align="center">&nbsp;</p>to bind a result of *exp* to the *variable*<br>to bind `10` (Host Expression) to the *num*<br>a binding combined with parsing property<br>a binding to the [pseudo-variable](#pseudo-variables), e.g.:<br>the [result binding](#result-binding)|
