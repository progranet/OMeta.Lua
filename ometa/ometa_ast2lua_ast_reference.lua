...
local clone, n = Array({}), 0
for i = 1, #node do
local l = toLuaAst(node[i], context)
if Array:isInstance(l) then
for li = 1, #l do
n = n + 1
clone[n] = l[li]
end
...
name = GetProperty({context = Get({name = Name({'input'})}), index = Name({'stream'})})
else
error('unknown or read only pseudo-variable: "' .. name .. '"')
end
end
body:append(stat([[__pass__, ]], name, [[ = ]], res, [[]]))
context.locals['__pass__'] = true
if Name:isInstance(name) then
